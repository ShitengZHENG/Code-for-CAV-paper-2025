function objfun=fun_SimAVTraffic_Pre4_forGA(paraUpper,para_AV,LowLevelModel_label)
    ...
    for t=1:step-1
        ...
        for j=2:nveh
            ...
            if j==2
                ...
                acmd(t+1,j)=(ka4_+ka3_+ka2_+ka1_)*acmd(t-dd,j-1)+...
                    (kv4_+kv3_+kv2_+kv1_)*dv1(j)+...
                    kg_*(dx(j)-dx_desired);
            elseif j==3
                dx_desired=Tg_*vdata(t,j)+Gmin;
                acmd(t+1,j)=(ka4_+ka3_+ka2_)*acmd(t-dd,j-2)+ka1_*acmd(t-dd,j-1)+...
                    (kv4_+kv3_+kv2_)*dv2(j)+kv1_*dv1(j)+...
                    kg_*(dx(j)-dx_desired);
            elseif j==4
                dx_desired=Tg_*vdata(t,j)+Gmin;
                acmd(t+1,j)=(ka4_+ka3_)*acmd(t-dd,j-3)+ka2_*acmd(t-dd,j-2)+ka1_*acmd(t-dd,j-1)+...
                    (kv4_+kv3_)*dv3(j)+kv2_*dv2(j)+kv1_*dv1(j)+...
                    kg_*(dx(j)-dx_desired);
            else
                dx_desired=Tg_*vdata(t,j)+Gmin;
                acmd(t+1,j)=ka4_*acmd(t-dd,j-4)+ka3_*acmd(t-dd,j-3)+ka2_*acmd(t-dd,j-2)+ka1_*acmd(t-dd,j-1)+...
                    kv4_*dv4(j)+kv3_*dv3(j)+kv2_*dv2(j)+kv1_*dv1(j)+...
                    kg_*(dx(j)-dx_desired);
            end
            if LowLevelModel_label==0
                K=X(1);theta=X(2);omega=X(3);td=X(4);%td=round(Td/dt);
                if t<td+1
                    adata(t+1,j)=0;
                else
