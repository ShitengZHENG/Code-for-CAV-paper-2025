function [no_platLeader,vdata,xdata,adata,acmd]=fun_SimMultiPlatoon_npre4(data_Input,para_AV,paraUpper,LowLevelModel_label,NoiseStrength,platsize)

L_veh=4.835;
kv1_=paraUpper(1);kv2_=paraUpper(2);kv3_=paraUpper(3);kv4_=paraUpper(4);
kg_=paraUpper(5);
ka1_=paraUpper(6);ka2_=paraUpper(7);ka3_=paraUpper(8);ka4_=paraUpper(9);
Tg_=paraUpper(10);
Gmin0=paraUpper(11);
Gmin=L_veh+Gmin0;
...
for t=1:step-1
    ...
    for nj=2:nveh
        ...
        elseif j==2
            ...
            acmd(t+1,nj)=ka3_*acmd(t-dd,nj-1)+ka2_*acmd(t-dd,nj-1)+ka1_*acmd(t-dd,nj-1)+...
                kv3_*dv1(nj)+kv2_*dv1(nj)+kv1_*dv1(nj)+...
                kg_*(dx(nj)-dx_desired);
        elseif j==3
            dx_desired=Tg_*vdata(t,nj)+Gmin;
            acmd(t+1,nj)=ka3_*acmd(t-dd,nj-2)+ka2_*acmd(t-dd,nj-2)+ka1_*acmd(t-dd,nj-1)+...
                kv3_*dv2(nj)+kv2_*dv2(nj)+kv1_*dv1(nj)+...
                kg_*(dx(nj)-dx_desired);
        elseif j==4
            dx_desired=Tg_*vdata(t,nj)+Gmin;
            acmd(t+1,nj)=(ka4_+ka3_)*acmd(t-dd,nj-3)+ka2_*acmd(t-dd,nj-2)+ka1_*acmd(t-dd,nj-1)+...
                (kv4_+kv3_)*dv3(nj)+kv2_*dv2(nj)+kv1_*dv1(nj)+...
                kg_*(dx(nj)-dx_desired);
        else
            dx_desired=Tg_*vdata(t,nj)+Gmin;
            acmd(t+1,nj)=ka4_*acmd(t-dd,nj-4)+ka3_*acmd(t-dd,nj-3)+ka2_*acmd(t-dd,nj-2)+ka1_*acmd(t-dd,nj-1)+...
                kv4_*dv4(nj)+kv3_*dv3(nj)+kv2_*dv2(nj)+kv1_*dv1(nj)+...
                kg_*(dx(nj)-dx_desired);
        end
